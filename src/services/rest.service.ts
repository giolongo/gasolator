...
export class RestService {
  ...
  public getOsrmRoute(startLon: number, startLat: number, endLon: number, endLat: number): Observable<{ routes: { geometry: string, distance: number }[] }> {
    return this.http.get<{ routes: { geometry: string, distance: number }[] }>(`https://router.project-osrm.org/route/v1/driving/${startLon},${startLat};${endLon},${endLat}?overview=full&geometries=polyline`)
  }
